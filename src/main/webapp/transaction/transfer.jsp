<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ page import="com.bank.bean.Account,com.bank.dto.SessionUser,com.bank.util.HtmlUtil,java.math.BigDecimal,java.math.RoundingMode,java.util.List" %>
<%!
    private String moneyText(BigDecimal amount) {
        if (amount == null) {
            return "0.00";
        }
        return amount.setScale(2, RoundingMode.HALF_UP).toPlainString();
    }
%>
<%
    SessionUser loginUser = (SessionUser) session.getAttribute("loginUser");
    List<Account> accounts = (List<Account>) request.getAttribute("accounts");
    String error = (String) request.getAttribute("error");
    Long selectedAccountId = (Long) request.getAttribute("selectedAccountId");
    String toAccountNo = (String) request.getAttribute("toAccountNo");
    String amount = (String) request.getAttribute("amount");
    String remark = (String) request.getAttribute("remark");
    request.setAttribute("activeNav", "transfer");
%>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    <title>本行转账 - iBank</title>
    <link rel="stylesheet" href="<%= request.getContextPath() %>/assets/css/main.css">
</head>
<body class="customer-page">
<%@ include file="/WEB-INF/jsp/customerTopbar.jspf" %>

<main class="layout">
    <section class="compact-page-head">
        <div class="page-heading">
            <p class="eyebrow">Transfer</p>
            <h1>本行转账</h1>
            <p class="muted">同一事务完成付款扣款、收款入账和双方流水落账。</p>
        </div>
        <a class="button secondary compact" href="<%= request.getContextPath() %>/transactions?txnType=TRANSFER_INNER">查看转账流水</a>
    </section>

    <% if (error != null) { %>
        <div class="alert danger"><%= HtmlUtil.escape(error) %></div>
    <% } %>

    <div class="care-note">
        <div>
            <strong>转账前先确认收款账号和用途。</strong>
            <p>提交后系统会同时完成付款扣款、收款入账、双方流水落账和风险限额校验；如果触发异常提醒，会告诉你原因和下一步。</p>
        </div>
    </div>

    <section class="task-layout">
        <article class="task-panel with-warm-accent">
            <form method="post" action="<%= request.getContextPath() %>/transfer" class="form">
                <label>
                    <span>付款账户</span>
                    <select name="fromAccountId" required>
                        <option value="">请选择账户</option>
                        <% if (accounts != null) {
                            for (Account account : accounts) {
                                boolean selected = selectedAccountId != null && selectedAccountId.equals(account.getAccountId());
                        %>
                            <option value="<%= account.getAccountId() %>" <%= selected ? "selected" : "" %>>
                                <%= HtmlUtil.escape(account.getAccountNo()) %>｜可用余额 ¥ <%= moneyText(account.getAvailableBalance()) %>
                            </option>
                        <%  }
                        } %>
                    </select>
                </label>
                <label>
                    <span>收款账号</span>
                    <input name="toAccountNo" type="text" maxlength="32" value="<%= HtmlUtil.escape(toAccountNo) %>" required>
                </label>
                <label>
                    <span>转账金额</span>
                    <input name="amount" type="number" min="0.01" max="200000" step="0.01" value="<%= HtmlUtil.escape(amount) %>" required>
                </label>
                <label>
                    <span>支付密码</span>
                    <input name="payPassword" type="password" required maxlength="6" pattern="\d{6}" inputmode="numeric">
                </label>
                <label>
                    <span>备注</span>
                    <input name="remark" type="text" maxlength="255" value="<%= HtmlUtil.escape(remark) %>">
                </label>
                <button class="button primary full" type="submit">提交转账</button>
            </form>
        </article>

        <aside class="task-side">
            <section class="content-section">
                <h2>提交前确认</h2>
                <ul class="task-checklist">
                    <li>收款账号必须是 iBank 已存在账户。</li>
                    <li>单笔最高 200000 元，仍会经过客户限额和风控规则校验。</li>
                    <li>成功后会生成一笔付款流水和一笔收款流水。</li>
                    <li>支付密码为 6 位数字，未设置时需先进入安全中心。</li>
                </ul>
            </section>
            <section class="content-section">
                <h2>账户余额</h2>
                <div class="movement-list">
                    <% if (accounts == null || accounts.isEmpty()) { %>
                        <div class="human-empty">
                            <strong>暂时没有可用于转账的账户。</strong>
                            <p>开通账户或完成入账后，可用余额会在这里展示，再选择付款账户提交转账。</p>
                        </div>
                    <% } else {
                        for (Account account : accounts) {
                    %>
                        <div class="movement-item">
                            <div class="movement-item-main">
                                <strong><%= HtmlUtil.escape(account.getAccountNo()) %></strong>
                                <p><%= HtmlUtil.escape(account.getAccountType()) %></p>
                            </div>
                            <strong>¥ <%= moneyText(account.getAvailableBalance()) %></strong>
                        </div>
                    <%  }
                    } %>
                </div>
            </section>
        </aside>
    </section>
</main>
</body>
</html>
